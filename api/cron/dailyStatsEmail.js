...
/**
 * Cron job endpoint for sending daily stats email at 11:59 PM
 * This is called automatically by Vercel Cron
 */
export default async function handler(req, res) {
  // Verify it's a legitimate cron call
  // Vercel Cron sends Authorization header with CRON_SECRET value
  const authHeader = req.headers.authorization;
  
  // Check for cron secret - Vercel sends it as "Bearer <secret>" or just the secret
  if (CRON_SECRET) {
    ...
    // If CRON_SECRET is not set, allow the call (less secure but works for testing)
    console.warn('⚠️ CRON_SECRET not configured - cron endpoint is unsecured');
  }

  // Allow GET (Vercel Cron default) or POST
  ...
  try {
    ...
    console.log('📅 Daily stats cron job triggered at', new Date().toISOString());
    ...
    // Create HTML email (same format as manual endpoint)
      ...
      <body>
        <div class="container">
          <div class="header">
            ...
            <p>${today}</p>
            <p style="font-size: 14px; margin-top: 10px; opacity: 0.8;">EcoRewards System - Automated Daily Summary (11:59 PM)</p>
          </div>
          ...
          <div class="footer">
            ...
            <p>This is an automated daily stats report sent at 11:59 PM</p>
    ...
    if (!resend) {
      ...
      return res.status(200).json({ 
        success: true, 
        message: 'Daily stats generated but email service not configured',
        data: {
    ...
    try {
      const emailResult = await resend.emails.send({
        from: FROM_EMAIL,
        to: ADMIN_EMAIL,
        subject: `📊 Daily Stats Report - ${today} (Automated)`,
        html: emailHTML,
      ...
      if (emailResult.error) {
        ...
        emailSent = true;
        console.log('✅ Daily stats email sent successfully via Resend:', emailResult.data?.id);
      }
    } catch (error) {
